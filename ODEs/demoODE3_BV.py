...
def costFunction(W, x, bc1, bc2):
    ...
    for xi in x:
        ...
        psy = A * (1 - xi) + B * xi + xi * (1 - xi) * nnOut
        psy1Deriv = -A + B + (1 - 2*xi) * nnOut + xi * (1 - xi) * nn1DerivOut
        psy2Deriv = -2 * nnOut + (2 - 4*xi) * nn1DerivOut + xi * (1 - xi) * nn2DerivOut
...
def resultSol(W, x):
    sol = x * np.sin(1) * np.exp(-1./5.) + x * (1 - x) * NN(W, x)[0][0]
    return sol
...
for i in range(1000):
    costGrad =  grad(costFunction)(W, X, 0, np.sin(1) * np.exp(-1./5.))
...
yRes = [xi*np.sin(1)*np.exp(-1./5.) + xi*(1 - xi)*NN(W, xi)[0][0] for xi in XPlot] 

# calculate relative error max norm
relErrMax = errorEval(funcDiff, 0, 1, 100, W, 1)
