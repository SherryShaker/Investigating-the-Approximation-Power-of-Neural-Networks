...
def partial_derivative_W_helper(W, xi, yi, k):
    w = W[0]
    v = W[1]
    b = W[2]
    ...
    # exclude points on the boundary for the derivative error sum calculation
    if (k == 't'):
...
def partial_derivative_W(W, T, S):
    ...
    for ti in T:
        xi, yi = ti
        cost_deriv_1 += partial_derivative_W_helper(W, xi, yi, 't')
    # cost2: costs grad related to points in set S (on the boundary)
    cost_deriv_2 = np.zeros(W[0].shape, dtype=np.float)
    for si in S:
        xi, yi = si
        cost_deriv_2 += partial_derivative_W_helper(W, xi, yi, 's')
    result = cost_deriv_1 + cost_deriv_2
    return result

def partial_derivative_V_helper(W, xi, yi, k):
    w = W[0]
    v = W[1]
    b = W[2]
...
def partial_derivative_V(W, T, S):
    ...
    for ti in T:
        xi, yi = ti
        cost_deriv_1 += partial_derivative_V_helper(W, xi, yi, 't')
    # cost2: costs grad related to points in set S (on the boundary)
    cost_deriv_2 = np.zeros(W[1].shape, dtype=np.float)
    for si in S:
        xi, yi = si
        cost_deriv_2 += partial_derivative_V_helper(W, xi, yi, 's')
    result = cost_deriv_1 + cost_deriv_2
    return result
